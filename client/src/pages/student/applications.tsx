...
export default function StudentApplications() {
  ...
  useEffect(() => {
    console.log("[DEBUG] StudentApplications mounted");
  ...
  return data; // { path: ..., originalName: ... }
};
const handleShareDocument = async (file: File, applicationId: number, companyId: number) => {
  try {
    // 1. Upload du fichier
    const uploaded = await handleFileUpload(file);

    // 2. Partage avec l’entreprise
    const res = await fetch("http://localhost:8080/api/documents/share", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
      body: JSON.stringify({
        ...
        documentType: "Document partagé",
        path: uploaded.path,
        message: "Voici mon document complémentaire",
      }),
    });
    ...
    toast({
      title: "Erreur",
      description: error.message || "Le partage a échoué.",
      variant: "destructive",
    });
  }
};
  ...
  queryFn: async () => {
    console.log('[QUERY] Running queryFn for /api/applications/student');
    try {
      const response = await axiosInstance.get('/api/applications/student', {
      });
      return response.data || [];
    } catch (error) {
      console.warn('[QUERY] Using mock applications data due to:', error.message);
      return [
        {
          id: 1,
          internshipId: 1,
  ...
  enabled: !!user?.id, // Enable query only if user ID is available
  staleTime: 0,
  onError: (error) => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      try {
        ...
        console.warn("[QUERY] Using mock internships data due to:", error.message);
        return [
          {
            id: 1,
    ...
    enabled: true, // Temporary for testing
    staleTime: 0,
    onError: (error) => {
      console.error("[QUERY] Error:", error.message);
      toast({
  ...
  // Debug render state
  useEffect(() => {
    console.log("[RENDER] isLoadingApplications:", isLoadingApplications);
    console.log("[RENDER] applicationsError:", applicationsError);
    console.log("[RENDER] applications:", applications);
    console.log("[RENDER] isLoadingInternships:", isLoadingInternships);
    console.log("[RENDER] internshipsError:", internshipsError);
    console.log("[RENDER] internships:", internships);
  }, [isLoadingApplications, applicationsError, applications, isLoadingInternships, internshipsError, internships]);
  ...
  useEffect(() => {
      console.log("[DEBUG] Utilisateur connecté :", user); // ou context utilisateur
    }, [user]);
...
console.log("[DEBUG] Applications reçues du backend:", applications);
console.log("[DEBUG] Applications filtrées:", filteredApplications);
