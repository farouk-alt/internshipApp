...
export default function SchoolMessages() {
  ...
  console.log("SchoolMessages - user from useAuth:", user);

  // Récupérer les contacts (entreprises et étudiants)
  const { data: contacts, isLoading: isLoadingContacts } = useQuery<Contact[]>({
    queryKey: ["/api/school/contacts"],
    enabled: !!user,
    ...
    if (selectedContact && user) {
      ...
      const markAsRead = async () => {
        try {
          ...
          queryClient.invalidateQueries({ queryKey: ["/api/school/contacts"] });
        } catch (error) {
  ...
  // Helper pour déterminer le type de contact
                  ...
                  >
                    ...
                    <div className="ml-3 flex-1 min-w-0">
                      ...
                      <p className="text-sm text-gray-500 truncate">
                        {contact.lastMessage || getContactType(contact.userType)}
                      </p>
                    </div>
                  </div>
                ))
              ...
              <div className="p-4 border-b flex items-center">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {getContactType(selectedContact.userType)}
                  </p>
                </div>
              </div>
